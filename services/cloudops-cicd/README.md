...
`/observability` 会关联 Rollout 灰度阶段（phase、step、stable/canary 权重）与 Istio Prometheus 指标（请求速率、5xx 速率、P95 延迟、按 destination 拆分）。`/release`、`/records` 和 snapshot 会附带 `observability` 字段，便于故障复盘时对照灰度阶段与流量指标。
